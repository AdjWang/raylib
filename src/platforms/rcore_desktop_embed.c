/**********************************************************************************************
*
*   rcore_desktop_embed - Functions to manage window, graphics device and inputs
*
*   PLATFORM: DESKTOP: EMBED
*       - Windows (Win32)
*
*   LIMITATIONS:
...
// Module Internal Functions Declaration
//----------------------------------------------------------------------------------
int InitPlatform(void* user);             // Initialize platform (graphics, inputs and more)
...
{
    // Embed backend does not handle graphic, get framebuffer directly from rlgl.
}
...
int InitPlatform(void* user)
{
    ...
    CORE.Window.flags = platform.desiredFlags;
...
*/
    ...
    // Chain wndproc.
    platform.originalWndProc = (WNDPROC)SetWindowLongPtr(platform.hwnd, GWLP_WNDPROC, (LONG_PTR)WndProc);
...
void ClosePlatform(void)
{
    // Detach.
    platform.hwnd = NULL;
    platform.originalWndProc = NULL;
}

static LRESULT WndProcFallthrough(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam) {
  if (platform.originalWndProc) {
    return CallWindowProc(platform.originalWndProc, hwnd, msg, wparam, lparam);
  } else {
